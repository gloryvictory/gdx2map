...
const googleTiles = (lyrs: string) => GOOGLE_SUBS.map((s) => `https://${s}.google.com/vt/lyrs=${lyrs}&x={x}&y={y}&z={z}`)

export const GOOGLE_ROAD_STYLE = createXyzRasterStyle(googleTiles('m'), '© Google')
export const GOOGLE_SATELLITE_STYLE = createXyzRasterStyle(googleTiles('s'), '© Google')
export const GOOGLE_HYBRID_STYLE = createXyzRasterStyle(googleTiles('y'), '© Google')
export const GOOGLE_TERRAIN_STYLE = createXyzRasterStyle(googleTiles('t'), '© Google')
